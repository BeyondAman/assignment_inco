...
6.  [Access Control & Security Best Practices](#6-access-control--security-best-practices)
...
* **Example Bucket Structure:**
    ```
    ├── dev/
    │   ├── Storage_Bucket1_dev  (for /auth content)
    │   ├── Storage_Bucket2_dev  (for /info content)
    │   └── Storage_Bucket3_dev  (for /customers content)
    │
    ├── staging/
    │   ├── Storage_Bucket1_staging (for /auth content)
    │   ├── Storage_Bucket2_staging (for /info content)
    │   └── Storage_Bucket3_staging (for /customers content)
    │
    └── prod/
        ├── Storage_Bucket1_prod (for /auth content)
        ├── Storage_Bucket2_prod (for /info content)
        └── Storage_Bucket3_prod (for /customers content)
    ```
...
2.  **Initialize Backend Storage (One-Time Setup per environment):**
    ...
    * Navigate to the `backend-storage-setup` directory:
        ```bash
        cd backend-storage-setup
        ```
